...
#stick with FC -> Relu(FC) from 0005 but add raw input spectra to flattened UNet outputs
...
#Concat on the channel dimension =1
        x19 = torch.cat([x18, spec], dim=1)
#Now with effectively 84 channels, flatten those channels
        x20 = x19.view(-1, self.num_flatten)
        x21 = self.fc1(x20)
        x22 = F.dropout(x21, self.dropout_rate)
        x23 = F.relu(self.fc2(x22))
        return x23